...
# ─── TLAČÍTKO OTEVŘÍT FORMULÁŘ ─────────────────────────────────────────────────
class OpenModalButton(discord.ui.View):
    def __init__(self, category_key: str):
        super().__init__(timeout=60)
        self.category_key = category_key

    @discord.ui.button(label="📋 Otevřít formulář", style=discord.ButtonStyle.green)
...
class CategorySelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        cat = CATEGORIES[category_key]
        view = OpenModalButton(category_key)
        await interaction.response.send_message(
            f"Vybrals kategorii **{cat['emoji']} {cat['label']}**.\nKlikni na tlačítko níže pro otevření formuláře:",
            view=view,
            ephemeral=True
        )
