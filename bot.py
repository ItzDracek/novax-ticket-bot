    ...
    async for msg in channel.history(limit=1000, oldest_first=True):
        ...
        for att in msg.attachments:
            if att.content_type and att.content_type.startswith("image"):
                ...
                if img_b64:
                    attachments_html += f'<div style="margin-top:4px;"><img src="{img_b64}" style="max-width:400px;max-height:300px;border-radius:4px;"></div>'
                else:
        ...
        for e in msg.embeds:
            ...
            if e.image:
                ...
                embeds_html += f'<div style="margin-top:4px;"><img src="{src}" style="max-width:400px;border-radius:4px;"></div>'
            embeds_html += '</div>'
...
# ─── POTVRZENÍ ZAVŘENÍ (veřejné v kanálu) ──────────────────────────────────────
class ConfirmCloseView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=60)

    @discord.ui.button(label="✅ Uzavřít", style=discord.ButtonStyle.danger, custom_id="confirm_close_yes")
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(content="🔒 Ticket se uzavře za **10 sekund**...", view=self)
        await send_transcript(interaction.channel, interaction.user)
        await asyncio.sleep(10)
        await interaction.channel.delete()

    @discord.ui.button(label="❌ Zrušit", style=discord.ButtonStyle.secondary, custom_id="confirm_close_no")
