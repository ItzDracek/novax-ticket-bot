...
async def send_transcript(channel: discord.TextChannel, closer: discord.Member):
    ...
    async for msg in channel.history(limit=500, oldest_first=True):
        ...
        for e in msg.embeds:
            if e.title:
                embeds_text += f"[EMBED: {e.title}] "
            for field in e.fields:
                embeds_text += f"{field.name}: {field.value} "
        messages.append(f"[{time}] {msg.author.display_name}: {content} {embeds_text}".strip())
...
h2 {{ color: #7289da; }}
.msg {{ padding: 6px 0; border-bottom: 1px solid #40444b; }}
.author {{ color: #7289da; font-weight: bold; }}
.time {{ color: #72767d; font-size: 0.8em; margin-left: 8px; }}
...
"""
    async for msg in channel.history(limit=500, oldest_first=True):
        ...
        for e in msg.embeds:
            if e.title:
                content += f'<i>[Embed: {e.title}]</i> '
        html += f'<div class="msg"><span class="author">{msg.author.display_name}</span><span class="time">{time}</span><div class="content">{content}</div></div>\n'
    ...
    if transcript_channel:
        file = discord.File(
            fp=__import__("io").BytesIO(html.encode("utf-8")),
...
class CloseView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send("🔒 Ticket se zavírá...", ephemeral=True)
...
class CategorySelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Reset výběru
        self.values.clear() if hasattr(self.values, 'clear') else None
