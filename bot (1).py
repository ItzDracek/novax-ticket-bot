...
async def send_transcript(channel: discord.TextChannel, closer: discord.Member):
    ...
    async for msg in channel.history(limit=1000, oldest_first=True):
        ...
        # Přílohy (obrázky, videa, soubory)
        attachments_html = ""
        for att in msg.attachments:
            if att.content_type and att.content_type.startswith("image"):
                attachments_html += f'<div style="margin-top:4px;"><img src="{att.url}" style="max-width:400px;max-height:300px;border-radius:4px;"></div>'
            elif att.content_type and att.content_type.startswith("video"):
                attachments_html += f'<div style="margin-top:4px;"><video controls style="max-width:400px;"><source src="{att.url}"></video></div>'
            else:
                attachments_html += f'<div style="margin-top:4px;"><a href="{att.url}" style="color:#7289da;">{att.filename}</a></div>'
        ...
        for e in msg.embeds:
            ...
            if e.image:
                embeds_html += f'<div style="margin-top:4px;"><img src="{e.image.url}" style="max-width:400px;border-radius:4px;"></div>'
            embeds_html += '</div>'
...
class ConfirmCloseView(discord.ui.View):
    ...
    @discord.ui.button(label="✅ Uzavřít", style=discord.ButtonStyle.danger, custom_id="confirm_close")
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("🔒 Ticket se uzavře za **10 sekund**...", ephemeral=False)
        await send_transcript(interaction.channel, interaction.user)
        await asyncio.sleep(10)
        await interaction.channel.delete()

    @discord.ui.button(label="❌ Zrušit", style=discord.ButtonStyle.secondary, custom_id="cancel_close")
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("✅ Uzavření ticketu bylo zrušeno.", ephemeral=True)
        self.stop()
...
class CloseView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            "⚠️ Opravdu chceš uzavřít tento ticket?",
