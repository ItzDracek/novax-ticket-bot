...
class CategoryView(discord.ui.View):
    ...
    @discord.ui.button(label="❓ Ostatní", style=discord.ButtonStyle.secondary, custom_id="cat_ostatni", row=0)
    async def btn_ostatni(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(build_modal("ostatni"))

    @discord.ui.button(label="🏛️ Žádost o frakci", style=discord.ButtonStyle.primary, custom_id="cat_zadost_frakci", row=0)
    async def btn_zadost_frakci(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(build_modal("zadost_frakci"))

    @discord.ui.button(label="🐛 Bugy/Herní chyby", style=discord.ButtonStyle.success, custom_id="cat_bugy", row=1)
    async def btn_bugy(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(build_modal("bugy"))

    @discord.ui.button(label="⚠️ Stížnost na ST", style=discord.ButtonStyle.danger, custom_id="cat_stiznost_staff", row=1)
    async def btn_stiznost(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(build_modal("stiznost_staff"))

    @discord.ui.button(label="🚨 Nahlášení hráče", style=discord.ButtonStyle.danger, custom_id="cat_nahlasenihrace", row=2)
    async def btn_nahlasenihrace(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(build_modal("nahlasenihrace"))
