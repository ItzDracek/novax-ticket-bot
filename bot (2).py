...
async def send_transcript(channel: discord.TextChannel, closer: discord.Member):
    ...
    async for msg in channel.history(limit=1000, oldest_first=True):
        ...
        for att in msg.attachments:
            ...
            if att.content_type and att.content_type.startswith("image") or any(fname_lower.endswith(ext) for ext in IMAGE_EXTENSIONS):
            ...
            elif att.content_type and att.content_type.startswith("video") or any(fname_lower.endswith(ext) for ext in VIDEO_EXTENSIONS):
                attachments_html += (
                    f'<div style="margin-top:6px;">'
                    f'<video controls style="max-width:400px;max-height:300px;border-radius:6px;display:block;">'
                    f'<source src="{att.url}">'
                    f'</video>'
                    f'<div style="font-size:0.75em;color:#72767d;margin-top:2px;">🎥 <a href="{att.url}" style="color:#7289da;" target="_blank">{att.filename}</a></div>'
                    f'</div>'
                )
            else:
                attachments_html += f'<div style="margin-top:4px;">📎 <a href="{att.url}" style="color:#7289da;" target="_blank">{att.filename}</a></div>'
...
class ConfirmCloseView(discord.ui.View):
    ...
    @discord.ui.button(label="Uzavřít", style=discord.ButtonStyle.danger, custom_id="confirm_close_yes", emoji="🔒")
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(
            content="🔒 Ticket se uzavře za **5 sekund**...",
            view=self
        )
    ...
    @discord.ui.button(label="Zrušit", style=discord.ButtonStyle.secondary, custom_id="confirm_close_no", emoji="❌")
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(content="✅ Uzavření ticketu bylo zrušeno.", view=self)
        self.stop()
